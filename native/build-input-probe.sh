...
"$llvm_bin/clang-cl" \
  ...
  /Fo"$build_dir/input-probe.obj" \
  "$native_dir/input-probe.c"

"$llvm_bin/lld-link" \
  /nologo /machine:x64 /subsystem:native \
  /entry:NtProcessStartup /nodefaultlib \
  /stack:524288,12288 \
  /out:"$build_dir/inputprobe.exe" \
  "$build_dir/input-probe.obj" \
  "$sdk_root/um/x64/ntdll.lib"

"$llvm_bin/llvm-readobj" \
  --file-headers --coff-imports "$build_dir/inputprobe.exe"

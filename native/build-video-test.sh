...
"$llvm_bin/clang-cl" \
  ...
  /Fo"$build_dir/video-test.obj" \
  "$native_dir/video-test.c"

"$llvm_bin/lld-link" \
  /nologo /machine:x64 /subsystem:native \
  /entry:NtProcessStartup /nodefaultlib \
  /out:"$build_dir/video-test.exe" \
  "$build_dir/video-test.obj" \
  "$sdk_root/um/x64/ntdll.lib"

"$llvm_bin/llvm-readobj" --file-headers --coff-imports "$build_dir/video-test.exe"
